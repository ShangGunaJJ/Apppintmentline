...
class SimpleCalendar {
  ...
  create() {
    ...
    for (var i = 0; i < 35; i++) {
        days.innerHTML = days.innerHTML + '<div class="sc-item"><div class="day"></div><div class="lunar-day"></div><div class="setWorkDay">设置非工作日</div><div class="cancelWorkDay">取消非工作日</div></div>';
    }
    //添加下拉框数据
    ...
    if (options.showMark) {
      ...
      if (data) {
        daysElement.forEach(function (v, i) {
          ...
          var workday=data[year + '-' + (currentmonth>9?currentmonth:'0'+currentmonth) + '-' + (day>9?day:'0'+day)]
          if (workday) {
            v.classList.add('sc-mark');

            // 增强标签效果
            var reminder = v.querySelector('.lunar-day');
            v.title = reminder.innerText;
            reminder.innerText =workday; 
    ...
    daysElement.forEach(function (v, i) {
      ...
      v.onclick = function () {
        ...
        if (typeof calendar._options.onSelect === 'function') {
            calendar._options.onSelect(calendar.getSelectedDay(),v);
        }
      };
    });
